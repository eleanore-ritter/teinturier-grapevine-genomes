...
input="ragtag.scaffold.fasta" #Can set to empty and script will find fasta in directory submitted
...
do
	...
	then
		...
		echo "Running BWA for ${sample} to ${input} genome"
		if [ ${PE} = "TRUE" ]
		then
			bwa mem -t ${threads} \
	...
	then
		...
		echo "Marking duplicates for ${sample}-${input}"
		java ${java_options} -jar ${picard} MarkDuplicates \
			-I round_${a}.bam \
			-O round_${a}_md.bam \
			-M round_${a}_md_metrics.txt \
			--REMOVE_DUPLICATES TRUE
		echo "Indexing ${sample}-${input} marked duplicate bam"
	...
	then
		...
		java ${java_options} -jar ${pilon} \
			...
			--fix all \
			--output pilon_${a}
		echo "Creating bwa index for pilon_${a}.fasta"
		bwa index pilon_${a}.fasta
	fi
	echo "Creating bwa index for pilon_${a}.fasta"
