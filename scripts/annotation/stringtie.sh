...
export PATH="${conda}/envs/stringtie/bin:${PATH}"
export LD_LIBRARY_PATH="${conda}/envs/stringtie/lib:${LD_LIBRARY_PATH}"
...
# List of datasets is already made
SR_bam_list=$(cat ${sample}_short.txt)
echo ${SR_bam_list}

LR_bam_list=$(cat ${sample}_long.txt)
echo ${LR_bam_list}
